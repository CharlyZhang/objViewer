...
public:
	CZMat4()
	{	LoadIdentity();	}
	CZMat4(	float e0, float e1, float e2, float e3,
				float e4, float e5, float e6, float e7,
				float e8, float e9, float e10, float e11,
				float e12, float e13, float e14, float e15);
	CZMat4(const float * rhs);
	CZMat4(const CZMat4 & rhs);
	~CZMat4() {}	//empty
	...
	CZMat4 operator-(const CZMat4 & rhs) const;
	CZMat4 operator*(const CZMat4 & rhs) const;
	CZMat4 operator*(const float rhs) const;
	CZMat4 operator/(const float rhs) const;
	friend CZMat4 operator*(float scaleFactor, const CZMat4 & rhs);

	bool operator==(const CZMat4 & rhs) const;
	bool operator!=(const CZMat4 & rhs) const;

	//self-add etc
	void operator+=(const CZMat4 & rhs);
	void operator-=(const CZMat4 & rhs);
	void operator*=(const CZMat4 & rhs);
	void operator*=(const float rhs);
	void operator/=(const float rhs);
	...
	//unary operators
	CZMat4 operator-(void) const;
	CZMat4 operator+(void) const {return (*this);}
	...
	//Other methods
	void Invert(void);
	CZMat4 GetInverse(void) const;
	void Transpose(void);
	CZMat4 GetTranspose(void) const;
	void InvertTranspose(void);
	CZMat4 GetInverseTranspose(void) const;

	//Inverse of a rotation/translation only matrix
	void AffineInvert(void);
	CZMat4 GetAffineInverse(void) const;
