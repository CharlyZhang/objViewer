...
// CZMat4
CZMat4::CZMat4(float e0, float e1, float e2, float e3,
...
CZMat4::CZMat4(const CZMat4 & rhs)
{
	memcpy(entries, rhs.entries, 16*sizeof(float));
}

CZMat4::CZMat4(const float * rhs)
{
	memcpy(entries, rhs, 16*sizeof(float));
}

void CZMat4::SetEntry(int position, float value)
{
	if(position>=0 && position<=15)
		entries[position]=value;
}
	
float CZMat4::GetEntry(int position) const
{
	if(position>=0 && position<=15)
...
VECTOR4D CZMat4::GetRow(int position) const
{
	if(position==0)
...
VECTOR4D CZMat4::GetColumn(int position) const
{
	if(position==0)
...
CZMat4 CZMat4::operator*(const CZMat4 & rhs) const
{
	...
	{
		return CZMat4(	entries[0]*rhs.entries[0]+entries[4]*rhs.entries[1]+entries[8]*rhs.entries[2],
	...
	if(	entries[3]==0.0f && entries[7]==0.0f && entries[11]==0.0f && entries[15]==1.0f)
	{
		return CZMat4(	entries[0]*rhs.entries[0]+entries[4]*rhs.entries[1]+entries[8]*rhs.entries[2]+entries[12]*rhs.entries[3],
...
CZMat4 CZMat4::operator*(const float rhs) const
{
	return CZMat4(	entries[0]*rhs,
						entries[1]*rhs,
						entries[2]*rhs,
						entries[3]*rhs,
						entries[4]*rhs,
...
CZMat4 operator*(float scaleFactor, const CZMat4 & rhs)
{
	return rhs*scaleFactor;
}

bool CZMat4::operator==(const CZMat4 & rhs) const
{
	for(int i=0; i<16; i++)
	{
...
void CZMat4::operator+=(const CZMat4 & rhs)
{
	(*this)=(*this)+rhs;
}
...
void CZMat4::operator*=(const float rhs)
{
	(*this)=(*this)*rhs;
}
...
CZMat4 CZMat4::operator-(void) const
{
	CZMat4 result(*this);
...
VECTOR4D CZMat4::operator*(const VECTOR4D rhs) const
{
...
VECTOR3D CZMat4::operator*(const VECTOR3D rhs_) const
{
	...
	VECTOR4D result(	entries[0]*rhs.x
		+	entries[4]*rhs.y
		+	entries[8]*rhs.z
		+	entries[12]*rhs.w,
...
VECTOR3D CZMat4::GetInverseRotatedVector3D(const VECTOR3D & rhs) const
{
	//rotate by transpose:
...
VECTOR3D CZMat4::GetTranslatedVector3D(const VECTOR3D & rhs) const
{
	return VECTOR3D(rhs.x+entries[12], rhs.y+entries[13], rhs.z+entries[14]);
}

VECTOR3D CZMat4::GetInverseTranslatedVector3D(const VECTOR3D & rhs) const
{
	return VECTOR3D(rhs.x-entries[12], rhs.y-entries[13], rhs.z-entries[14]);
}

void CZMat4::Invert(void)
{
	*this=GetInverse();
}

CZMat4 CZMat4::GetInverse(void) const
{
	CZMat4 result=GetInverseTranspose();
...
void CZMat4::Transpose(void)
{
	*this=GetTranspose();
}

CZMat4 CZMat4::GetTranspose(void) const
{
	return CZMat4(	entries[ 0], entries[ 4], entries[ 8], entries[12],
...
void CZMat4::InvertTranspose(void)
{
	*this=GetInverseTranspose();
}

CZMat4 CZMat4::GetInverseTranspose(void) const
{
	CZMat4 result;
...
void CZMat4::AffineInvertTranspose(void)
{
	(*this)=GetAffineInverseTranspose();
}

CZMat4 CZMat4::GetAffineInverseTranspose(void) const
{
	//return the transpose of the rotation part
	//and the negative of the inverse rotated translation part
	//transposed
...
void CZMat4::SetTranslation(const VECTOR3D & translation)
{
	LoadIdentity();
...
void CZMat4::SetScale(const VECTOR3D & scaleFactor)
{
	LoadIdentity();
...
void CZMat4::SetUniformScale(const float scaleFactor)
{
	LoadIdentity();
...
void CZMat4::SetRotationAxis(const double angle, const VECTOR3D & axis)
{
	VECTOR3D u=axis.GetNormalized();
...
void CZMat4::SetRotationX(const double angle)
{
	LoadIdentity();
...
void CZMat4::SetRotationY(const double angle)
{
	LoadIdentity();
...
void CZMat4::SetRotationZ(const double angle)
{
	LoadIdentity();
...
void CZMat4::SetRotationEuler(const double angleX, const double angleY, const double angleZ)
{
	LoadIdentity();
...
void CZMat4::SetPerspective(	float left, float right, float bottom,
...
void CZMat4::SetPerspective(float fovy, float aspect, float n, float f)
{
...
void CZMat4::SetOrtho(	float left, float right, float bottom,
							float top, float n, float f)
{
	LoadIdentity();
...
void CZMat4::SetLookAt(float eyeX, float eyeY, float eyeZ, float centerX, float centerY, float centerZ, float upX, float upY, float upZ)
{
...
void CZMat4::SetRotationPartEuler(const double angleX, const double angleY, const double angleZ)
{
